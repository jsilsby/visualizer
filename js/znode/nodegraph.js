function NodeGraph(){
  ...
  function startDrag(element, bounds, dragCallback){
	...
	this.codeRepo = {};
	this.setCodeRepository = function(codeArray) {
		codeRepo = codeArray;
	}
  ...
  function Node(xp, yp, w, h, noDelete, forceId){
    ...
    src.css({"position":"absolute","padding-right" : 2, "padding-top" : 1, "padding-left" : 2,
              "color" : "white", "font-family" : "sans-serif",
              "top" : 0, "left": n.width()-20, "cursor": "pointer",
              "font-size" : "7px", "background-color" : "gray", "z-index" : 100});
    src.hover(function(){
		src.css("color","black");
    }, function(){
		...
		if (codeRepo) {
			$dialog = $('<div></div>').html("<pre>" + codeRepo[curr.id].src + "</pre>").dialog({
				autoOpen: false,
				title: codeRepo[curr.id].name,
				width: 600,
				height: 400
			});
			$dialog.dialog('open');
